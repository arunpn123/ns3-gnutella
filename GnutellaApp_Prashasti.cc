...
{
    // blaub: DONT flood queries after a hit is returned
    bool hit_mine = false;
...
//            NS_LOG_INFO("File name should be: " << result_set[i].shared_file_name);
    ...
    // blaub: if we sent a query hit, don't forward this query any more
    // and just assume the client's going to get our query hit
    // (a bad assumption?)
    if(hit_mine)
    {
